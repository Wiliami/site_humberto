...
$UpdateLevel = $_GET['level']; 
?>
<div class="container">
    <?php
    $Read = new Read();
    $Read->FullRead("SELECT * FROM users_levels WHERE level_id = :li", "li={$UpdateLevel}");
    if($Read->getResult()) {
        foreach($Read->getResult() as $level) {
            ?>
    <div class="d-sm-flex align-items-center justify-content-start mb-4">
        <i class="fas fa-layer-plus"></i>
        <h1 class="h3 mb-0 text-gray-800">Atualizar nível <?= $level['level_desc'] ?></h1>
    </div>
    <form action="" method="post">
        ...
        <input type="submit" class="btn btn-success mb-2" name="register_lesson" value="Atualizar nível">
    </form>
    <?php
        }
    } else {
        Error('Nenhum nível de usuário foi selecionado para atualizar!');
    }
    ?>
</div>
